...
and the vocab size (vocab_size). Then we use an embedding
...
class InputEmbeddings(nn.Module):
    ...
    def __init__(self, d_model: int, variables: int):
        super().__init__()
...
class PositionalEmbeddings(nn.Module):
    ...
    def forward(self, x):
        ...
        # the dimensions of the word embedding. Remember the dimensions are: [batch, pos_embeddings, dimension]
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        # We extract the d_k, which is the last dimension of Q, K, and V
        d_k = query.shape[-1]
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, encoder_out, src_mask, tgt_mask):
        ...
        x = self.residual_connections[1](x, lambda x: self.self_attention_block(x, encoder_out, encoder_out, src_mask))
...
class Transformer(nn.Module):
    ...
    def __init__(self, encoder: Encoder, decoder: Decoder, src_embed: InputEmbeddings,
                 ...
                 last_linear: LastLinear, output_dim: int):
        super().__init__()
    ...
    def generate(self, x, src_mask):
        ...
        tgt = torch.zeros_like(x[:, -1:, :])

        for _ in range(self.output_dim):

            output = self.decode(tgt, memory, src_mask, None)
