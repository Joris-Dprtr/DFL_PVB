...
class Tensors:
    ...
    def create_tensor(self):
        ...
        # we can't use the first [lags] + [gap] timesteps and remove the first forecast_period
        prediction_length = len(self.data) - self.lags - self.gap - self.forecast_period - self.flat_evaluation_length
        ...
        # Add back the forecast we removed in calculating the prediction_length
        predictions = prediction_length // divider + 1
        ...
        target_train, target_test, target_min, target_max = _scale(target_train_array_shaped, target_test_array_shaped,
                                                                   domain_min=self.domain_min[-1] if
                                                                   isinstance(self.domain_min, list) else None,
                                                                   domain_max=self.domain_max[-1] if
                                                                   isinstance(self.domain_max, list) else None)

        y_train = torch.tensor(target_train).type(torch.float32)
        y_test = torch.tensor(target_test).type(torch.float32)

        scalers.append([target_min, target_max])
        ...
        if self.flat_evaluation_length != 0:
            ...
            _, target_eval, _, _ = _scale(target_train_array_shaped, target_eval_array_shaped,
                                          domain_min=self.domain_min[-1] if
                                          isinstance(self.domain_min, list) else None,
                                          domain_max=self.domain_max[-1] if
                                          isinstance(self.domain_max, list) else None)
            ...
            return X_train, X_test, X_eval, y_train, y_test, y_eval, scalers
